...
def modify_in_record(sem,sec,new):
    ...
    if sem == '1' or sem == '2':
        ...
        year = 'fourth_year'
    # print(new,sem,sec)
    filename = f'D:\python\gits\ATTendancs\data/Attendance_xlsx/{year}_{sem}sem_IT{sec}.xlsx'
    # print(filename)
...
def Register_in_record():
    ...
    sec = sectionn.get()
    print(sectionn.get())
    wb = load_workbook(f'D:\python\gits\ATTendancs\data/Attendance_xlsx/{year}_{sem}sem_IT{sec}.xlsx')
    ...
    wb.save(f'D:\python\gits\ATTendancs\data/Attendance_xlsx/{year}_{sem}sem_IT{sec}.xlsx') 
...
def Retrive(roll,sem,year,section):
    ...
    filename = f'D:\python\gits\ATTendancs\data/Attendance_xlsx/{year}_{sem}sem_IT{section}.xlsx'
    df = pd.read_excel(filename,index=False)
    l = df[df['Enrollment']==roll].values.tolist()
...
def cancel():
    ...
    f3.pack(pady="115")
def update_details():
    new = [uname.get(),uenrollment.get(),usem.get(),usec.get(),ucontact.get(),uemail.get()]
    modify_in_record('5',section.get(),new)
...
def open_excel():
    ...
    file=f'D:\python\gits\ATTendancs\data\Attendance_xlsx/{year}_{sem}sem_IT{sec}.xlsx'   
    os.startfile(file)
